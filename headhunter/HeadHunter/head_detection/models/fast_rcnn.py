...
class GeneralizedRCNNTransform(nn.Module):
    ...
    # outputs = model(images)를 진행하게 되면, 여기에서 forward를 진행한다.
    # 따라서, images 파라미터에 들어가는게 바로 test.py의 그 images
    def forward(self, images, targets=None):
        for i in range(len(images)):
            image = images[i]
            target = targets[i] if targets is not None else targets
            if image.dim() != 3:           # 3차원인지 체크
                raise ValueError("images is expected to be a list of 3d tensors "   
                                 "of shape [C, H, W], got {}".format(image.shape))
            image = self.normalize(image)  # 정규화. 자기 자신 클래스의 함수는 self.xxx로 실행
